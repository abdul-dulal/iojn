...
const quickLinks = navLinks.filter((l) => !["Home", "Resources"].includes(l.label));
        ...
        <div className="grid gap-12 py-16 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1.3fr]">
          <div>
            ...
            <p className="mt-6 max-w-xs text-sm leading-relaxed text-white/60">
              The {site.fullName} is an innovative research support centre advancing impactful health,
              environmental and social research since {site.founded}.
            </p>
            <SocialLinks className="mt-8" />
          </div>

          <div>
            <h4 className="text-sm font-semibold tracking-wider text-white uppercase">Quick Links</h4>
            <ul className="mt-6 space-y-3">
              {quickLinks.map((l) => (
                <li key={l.href}>
                  <a href={l.href} className="text-sm text-white/60 transition-colors hover:text-cyan">
                    {l.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            <h4 className="text-sm font-semibold tracking-wider text-white uppercase">Research Services</h4>
            <ul className="mt-6 space-y-3">
              {serviceLinks.map((s) => (
                <li key={s}>
          ...
          <div>
            ...
            <ul className="mt-6 space-y-4 text-sm text-white/60">
              <li className="flex gap-3">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-cyan" /> {site.address}
              </li>
              <li>
                <a href={site.phoneHref} className="flex gap-3 transition-colors hover:text-cyan">
                  <Phone className="mt-0.5 h-4 w-4 shrink-0 text-cyan" /> {site.phone}
                </a>
              </li>
              <li>
                <a href={`mailto:${site.email}`} className="flex gap-3 transition-colors hover:text-cyan">
                  <Mail className="mt-0.5 h-4 w-4 shrink-0 text-cyan" /> {site.email}
                </a>
              </li>
            </ul>
          </div>
